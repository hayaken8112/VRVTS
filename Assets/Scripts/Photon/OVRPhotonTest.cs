...
public class OVRPhotonTest : MonoBehaviour {
	...
	void OnJoinedRoom(){
        ...
        int countWhenEntered = PhotonNetwork.room.PlayerCount -1;
        string sendContents = viewId + "_" + countWhenEntered;
    	PhotonNetwork.RaiseEvent(InstantiateVrAvatarEventCode, sendContents, true, new RaiseEventOptions() { CachingOption = EventCaching.AddToRoomCache, Receivers = ReceiverGroup.All });
    }

    Vector3[] place = new Vector3[] {new Vector3(0f, 1.7f, 0f), new Vector3(3f, 1.7f, 0f), new Vector3(5f, 1.7f, 0f)};
    ...
    private void OnEvent(byte eventcode, object content, int senderid)
	{
		if (eventcode == InstantiateVrAvatarEventCode)
		{
			...
			if (PhotonNetwork.player.ID == senderid)
			{
				...
				camera.transform.position = place[numberList];
				go = Instantiate(Resources.Load("LocalAvatar"), place[numberList], q) as GameObject;
				Debug.Log("アバターのpositionは；" + go.transform.position);
			}
			else
			{
				go = Instantiate(Resources.Load("RemoteAvatar"), place[numberList], q) as GameObject;
			}

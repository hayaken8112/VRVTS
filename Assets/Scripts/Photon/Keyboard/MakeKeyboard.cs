	...
	public void CreateKeyboard() {
		...
		for (int i = 0; i < c.Length; i++) {
			...
			if (0 <= i && i < 10) {
				...
				btn.transform.position = new Vector2(-300 + i * 55, 100);
				...
				btn.transform.position = new Vector2(-880 + i * 55, 40);
				btn.transform.SetParent(content, false);
				btn.transform.GetComponent<Button>().onClick.AddListener(() => OnClick(cs));
			} else if (21 <= i && i < 30) {
				//ontent.sizeDelta = new Vector2(-250 + i * 55, -20);
				btn.transform.position = new Vector2(-1405 + i * 55, -20);
				btn.transform.SetParent(content, false);
				btn.transform.GetComponent<Button>().onClick.AddListener(() => OnClick(cs));
			} else {
				//content.sizeDelta = new Vector2(-225 + i * 55, -80);
				btn.transform.position = new Vector2(-1850 + i * 55, -80);
				btn.transform.SetParent(content, false);
				btn.transform.GetComponent<Button>().onClick.AddListener(() => OnClick(cs));
			}
		}
